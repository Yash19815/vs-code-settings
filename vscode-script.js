...
/* VS Code puts the language id on every file label as a class:
   "<languageId>-lang-file-icon"  (e.g. "python-lang-file-icon").
   We read it from the active editor tab. */
function getActiveLanguageId() {
  const label =
    document.querySelector(".editor-group-container.active .tab.active .monaco-icon-label") ||
    document.querySelector(".tab.active .monaco-icon-label");
  if (!label) return null;
  const match = label.className.match(/([\w#+-]+)-lang-file-icon/i);
  return match ? match[1].toLowerCase() : null;
}

function applyWatermarkIcon() {
  const lang = getActiveLanguageId();
  if (lang) lastWatermarkLang = lang; // remember the last used language
